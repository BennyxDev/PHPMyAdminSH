    ...
    case $? in
      ...
      1 )
        ...
        while [ -z $dynuser ]; do
           ...
           read -ep $'\e[37mBitte gib ein Namen an mit dem du dich später in PHPmyAdmin Einlogst:\e[0m ' dynuser;
         ...
         export OPTIONS=("Passwort Generieren" "Passwort selbst festlegen")
         bashSelect
           case $? in
             0 )
...
function serverCheck() {
  ...
  if [[ $? != 127 ]]; then
    ...
    case $? in
      0 )
        status "Lösche mysql passwort"
        alreadyInstalled=true;
      ;;
      1 )
        status "Lösche MariaDB/MySQL"
  ...
  if [[ -d /usr/share/phpmyadmin ]]; then
    status "Es sieht so aus als hättest du schon PHPMyAdmin Installiert" "/"
...
function dbInstall(){
  ...
  status "Passwoerter werden generiert"
  ...
  status "Sichern MariaDB Installation"
...
function selfTest() {
  ...
  status "Test werden Gestartet"
  status "Starten von apache2 tests"
  ...
  if [[ "${MARIADB_TEST_PASSED}" != "true" ]]; then
    echo -e "${red}MariaDB Antwortet nicht bitte guck dir die Installation noch einmal an!"
    exit 1
  fi
...
function output() {
  ...
  if [[ $saveOutput == "true" ]]; then
    ...
    MariaDB-Daten:
       ...
       Port: 3306
       Benutzer: root
       Passwort: ${rootPasswordMariaDB} " > /root/.mariadbPhpma
    ...
    PHPMyAdmin-Daten:
      ...
      Benutzer: ${dynuser}
      Passwort: ${dynamicUserPassword}" > /root/.PHPma
      else
...
if [[ $EUID -ne 0 ]]; then
   echo "Dieses Script muss als benutzer gestartet werden" 1>&2
   exit 1
fi
...
while getopts ":sh" option; do
  case $option in
    h )
      echo "Das ist nur ein einfaches script um PHPMyAdmin, Apache2 and MariaDB on Debian based systems zu installieren."
      echo
      echo "Syntax: bash <(curl -s https://raw.githubusercontent.com/bennyxdev/PHPMyAdminSH/main/install.sh) [-h|-s]"
      echo
      echo "options:"
...
source <(curl -s https://raw.githubusercontent.com/bennyxdev/BashSelect.sh/main/BashSelect.sh)
